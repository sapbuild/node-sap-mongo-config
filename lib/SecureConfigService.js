...
function setConfigValue(service, key, value, context, noAudit) {
    var logger = getLogger(context), audit;
    var options = {
        upsert: true,
    ...
    if (!noAudit) {
        audit = getAudit(context);
    }
    return Promise.fnCall(
        function () {
            var encryptedData = encryptData(service.cipher, value);
            return {
                $set: {
        ...
        .then(function (update) {
            return new Promise(function (resolve, reject) {
                ...
                service.collection.findAndModify({ _id: key, v: { $lte: service.v } }, [
                        ['_id', 1]
                    ], update, options,
                    function (err, result) {
                        if (err) {
...
function migrateConfig(service, config, context) {
    ...
    return Promise.fnCall(decodeOldPasswords, service)
        .then(function (oldPwd) {
            ...
            return setConfigValue(service, key, value, context, true)
                .then(function () {
                    return {
                        _id: key,
                        value: value
...
function resetOldPwd(service, logger) {
    var decodedPwd = decodeOldPasswords(service);
    if (Object.keys(decodedPwd).length === 0) {
        return true;
    }
    delete service.decodedPwd;
    delete service.oldPwd;
    logger.debug('Deleting old passwords');
    return new Promise(
        function (resolve, reject) {
            service.collection.findAndModify({ _id: HEADER_KEY, v: service.v }, [
                    ['_id', 1]
                ], { $unset: { oldPwd: '' } }, { new: false },
                function (err) {
                    if (err) {
                        return reject(err);
                    }
                    logger.info('Old passwords successfully deleted');
                    resolve(true);
                });
        })
        .catch(function (err) {
...
SecureConfigService.prototype.set = function (key, value, context) {
    ...
    return setConfigValue(this, key, value, context);
};
...
SecureConfigService.prototype.migrateAll = function (context) {
    ...
    return new Promise(
        function (resolve, reject) {
            self.collection.find({ v: { $lt: self.v } }, { limit: 100 }).toArray(function (err, results) {
                if (err) {
                    return reject(err);
                }
        ...
        .then(function () {
            return resetOldPwd(self, logger);
        })
        .catch(function (err) {
